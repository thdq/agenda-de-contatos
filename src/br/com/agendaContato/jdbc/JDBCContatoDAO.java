...
public class JDBCContatoDAO implements ContatoDAO {
	...
	public Contato buscarPorId(int cod){
		...
		try {
			java.sql.Statement stmt = conexao.createStatement(); //Cria um objeto para enviar instruções SQL ao banco de dados.
			ResultSet rs = stmt.executeQuery(comando); // API JAVA que permite percorrermos uma consulta em um banco de dados
			while (rs.next()) {
				String nomeContato = rs.getString("nome");
				String endereco = rs.getString("endereco");
				int idContato = rs.getInt("idcontato");
	...
	public boolean atualizar(Contato contato) {
		...
		try {
			p = this.conexao.prepareStatement(comando); // armazena no objeto p a instruçao SQL
			p.setString(1, contato.getNome());
			p.setString(2, contato.getEndereco());
			p.setString(3, contato.getTelefone());
			
			p.executeUpdate(); // executa uma atualização no banco 
		} catch (SQLException e) {
			e.printStackTrace();
			return false;
		}
